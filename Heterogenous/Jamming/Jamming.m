function [xy_p_all, xy_c_all, L0_voxel, D_all_new] = Jamming(conid, xy_voxel, N, D_all, Gn, L0, Nvoxel_row, Kp, Kw_ratio, Kpw_ratio, test)
%{
Inputs: 
conid - index
xy_voxel - list of all unique voxels you could tessellate; size: number of
unique voxels x 2*N + 9 (x, y, xc, yc) 
N - number of particles per voxel
D_all - the small particle size for each unique voxel
Gn - the unit array of particle sizes  
%}
...
for i = 1:Nvoxel_row
    for j = 1:Nvoxel_row
        N_start = count * N;
        count = count + 1;
        D_all_new((count-1) * N + 1 : count * N) = D_all(conid(count)) * Gn;
        for n = 1:N         
            xy_p_all(n + N_start) = xy_p_new(conid(count), n) + (j - 1) * La(1) + (i - 1) * Lb(1);
            xy_p_all(n + N_start + Np) = xy_p_new(conid(count), n + N) + (j - 1) * La(2) + (i - 1) * Lb(2);
        end
    end
end
